    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center bg-[#F8FAFC]">
        <div className="text-center p-8 bg-white rounded-3xl shadow-sm border border-gray-100 max-w-md w-full mx-4">
          ...
          <h2 className="text-2xl font-800 text-gray-900 mb-2">Your cart is empty</h2>
          <p className="text-gray-500 mb-8">Looks like you haven't added any fresh groceries yet.</p>
          <Link href="/shop" className="block w-full bg-primary text-white font-bold py-4 rounded-xl hover:bg-secondary transition-colors">
            Start Shopping
          </Link>
        </div>
      </div>
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-4">
            {items.map((item) => (
              <div key={item.id} className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row items-start sm:items-center gap-6">
                ...
                {/* Product Image */}
                <Link href={`/product/${item.slug}`} className="w-full sm:w-24 h-48 sm:h-24 flex-shrink-0 bg-gray-50 rounded-xl p-2 border border-gray-100">
                  {item.images?.[0] ? (
                ...
                <div className="flex-1 min-w-0">
                  <Link href={`/product/${item.slug}`} className="font-bold text-gray-900 text-lg line-clamp-1 hover:text-primary transition-colors">
                    {item.name}
                  </Link>
          ...
          <div className="w-full lg:w-1/3">
            <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 sticky top-24">
              ...
              <div className="flex justify-between items-end mb-8">
                ...
                <span className="text-3xl font-900 text-gray-900 tracking-tight">${finalTotal.toFixed(2)}</span>
              </div>
