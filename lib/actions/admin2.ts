...
export async function adjustStock(
  ...
  try {
    ...
    const product = await prisma.product.findUnique({ where: { id: productId }, select: { slug: true, category: { select: { slug: true } } } });
    revalidatePath("/admin/inventory");
    revalidatePath("/shop");
    revalidatePath("/");
    if (product?.slug) revalidatePath(`/product/${product.slug}`);
    if (product?.category?.slug) revalidatePath(`/category/${product.category.slug}`);
    return { success: true };
  } catch (e: any) {
    return { success: false, error: e.message };
  }
...
export async function updateProductVariant(
  ...
  try {
    ...
    const product = await prisma.product.findUnique({ where: { id: variant.productId }, select: { slug: true, category: { select: { slug: true } } } });
...
export async function deleteProductVariant(id: string): Promise<ActionResult> {
  try {
    ...
    const product = variant?.productId ? await prisma.product.findUnique({ where: { id: variant.productId }, select: { slug: true, category: { select: { slug: true } } } }) : null;
