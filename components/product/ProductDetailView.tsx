...
import { createReview, deleteUserReview } from "@/lib/actions/reviews";
...
export function ProductDetailView({ 
  ...
  const variantOnSale = selectedVariant?.salePrice != null && selectedVariant.salePrice < selectedVariant.price;
  const productOnSale = product.originalPrice != null && product.price < product.originalPrice;

  const currentPrice = selectedVariant
    ? (variantOnSale ? selectedVariant.salePrice : selectedVariant.price)
    : (product.price ?? 0);
  
  const originalPrice = selectedVariant
    ? (variantOnSale ? selectedVariant.price : null)
    : (productOnSale ? product.originalPrice : null);
  ...
  return (
    <div className="section-container py-8 lg:py-12">
      <nav className="flex items-center gap-1.5 text-sm text-gray-400 mb-6">
        ...
        <ChevronRight className="w-3.5 h-3.5" />
        <span className="text-gray-700 font-medium line-clamp-1">{product?.name || "Product"}</span>
      </nav>
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 mb-16">
          ...
          {product.images.length > 1 && (
            <div className="flex gap-3 overflow-x-auto pb-2">
              {product.images.map((img: any, idx: number) => (
                ...
                >
                  <Image src={img.url} alt="thumbnail" width={80} height={80} className="w-full h-full object-contain mix-blend-multiply" unoptimized />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="w-full lg:w-1/2 flex flex-col">
          <div className="mb-2">
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">{product?.category?.name || "Uncategorized"}</span>
          </div>
          ...
          <div className="flex items-center gap-3 mb-6 pb-6 border-b border-gray-100 flex-wrap">
            ...
            <span className="text-sm text-gray-400">({product.reviewCount ?? product.reviews?.length ?? 0} Reviews)</span>
            <div className="w-1 h-1 bg-gray-300 rounded-full" />
            <span className="text-sm text-gray-500">{product.soldCount ?? 0} sold</span>
          </div>
          <div className="flex items-baseline gap-3 mb-6">
            <span className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">{formatPrice(currentPrice, settings.activeCurrency)}</span>
            {originalPrice && originalPrice > currentPrice && (
              <span className="text-xl text-gray-400 line-through">{formatPrice(originalPrice, settings.activeCurrency)}</span>
            )}
          ...
          <div className="bg-gray-50 rounded-2xl p-5 sm:p-6 mb-8 border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={cn("text-sm font-bold", inStock ? "text-blue-600" : "text-red-500")}>
      ...
      <div className="border-t border-gray-100 pt-12 lg:pt-16">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
          <div className="w-full lg:w-2/3">
            ...
            <div className="flex flex-col md:flex-row gap-8 mb-8 bg-gray-50 p-6 rounded-2xl border border-gray-100">
              {/* Rating Summary */}
              <div className="flex flex-col items-center justify-center min-w-[150px] border-b md:border-b-0 md:border-r border-gray-200 pb-6 md:pb-0 md:pr-6">
                <span className="text-5xl font-extrabold text-gray-900">{(product.rating ?? 0).toFixed(1)}</span>
                <div className="flex gap-1 text-yellow-400 my-2">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Star key={s} className={cn("w-5 h-5", s <= (product.rating ?? 0) ? "fill-current" : "text-gray-300")} />
                  ))}
                </div>
                <span className="text-sm text-gray-500">{product.reviewCount ?? product.reviews?.length ?? 0} reviews</span>
              </div>
              
              {/* Rating Breakdown */}
              <div className="flex-1 flex flex-col gap-2">
                {[5, 4, 3, 2, 1].map(stars => {
                  const count = ratingBreakdown[stars] ?? product.reviews?.filter((r: any) => r.rating === stars).length ?? 0;
                  const total = (product.reviewCount ?? product.reviews?.length) || 1;
                  const percentage = (count / total) * 100;
                  return (
                    <div key={stars} className="flex items-center gap-3 text-sm">
                      <div className="flex items-center gap-1 w-12 text-gray-600 font-medium">
                        {stars} <Star className="w-3.5 h-3.5 fill-current text-yellow-400" />
                      </div>
            ...
            {canReview && !existingReview && (
              <div className="mb-10 bg-white border border-gray-200 p-6 rounded-2xl shadow-sm">
                {!isWritingReview ? (
                  <div className="text-center">
                    <h3 className="text-lg font-bold text-gray-900 mb-2">Have you used this product?</h3>
                    <p className="text-gray-500 text-sm mb-4">Share your experience with others.</p>
                    <button 
                  ...
                  <div>
                      ...
                      <div className="flex gap-2 text-yellow-400 cursor-pointer">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star 
                            ...
                            className={cn("w-8 h-8", s <= reviewRating ? "fill-current" : "text-gray-300")} 
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3 justify-end">
                      ...
                      <button 
                        ...
                        className="bg-primary text-white px-6 py-2 rounded-xl font-semibold hover:bg-secondary transition-colors disabled:opacity-50"
                      >
            ...
            {existingReview && (
              <div className="mb-10 bg-primary/5 border border-primary/20 p-6 rounded-2xl relative">
                <div className="absolute top-4 right-4 flex gap-2">
                  <button 
                    onClick={async () => {
                      if (confirm("Are you sure you want to delete your review?")) {
                        const res = await deleteUserReview(existingReview.id);
                ...
                <h3 className="text-sm font-bold text-gray-900 mb-2 flex items-center gap-2">
                  Your Review
                  <span className="text-xs bg-primary text-white px-2 py-0.5 rounded-full font-medium">Published</span>
                </h3>
                <div className="flex gap-1 text-yellow-400 mb-2">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Star key={s} className={cn("w-4 h-4", s <= existingReview.rating ? "fill-current" : "text-gray-300")} />
                  ))}
                </div>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {review.verified && (
                            <span className="text-[10px] text-green-700 bg-green-100 px-1.5 py-0.5 rounded-full font-semibold flex items-center gap-1 mt-0.5 inline-flex">
            ...
            {relatedProducts.map((p: any) => (
              <Link key={p.id} href={`/product/${p.slug}`} className="group bg-white rounded-2xl border border-gray-100 p-3 hover:shadow-md transition-shadow">
                <div className="relative aspect-square bg-gray-50 rounded-xl mb-3 overflow-hidden">
                  {p.images?.[0] ? (
                    <Image src={p.images[0].url} alt={p.name} fill className="object-contain p-3 mix-blend-multiply group-hover:scale-105 transition-transform" unoptimized />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-gray-300">No Image</div>
                  )}
                </div>
                <p className="text-sm font-semibold text-gray-900 line-clamp-1">{p.name}</p>
                <p className="text-xs text-gray-400 mb-1">{p.category?.name || "Uncategorized"}</p>
                <p className="text-sm font-bold text-gray-900">{p.price != null ? formatPrice(p.price, settings.activeCurrency) : formatPrice(0, settings.activeCurrency)}</p>
              </Link>
            ))}
          </div>
        </div>
