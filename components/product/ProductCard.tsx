      ...
      <Link href={`/product/${product.slug}`} className="block">
        ...
        <div className="p-4">
          ...
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <span className="text-sm font-600 text-gray-800">{product.rating?.toFixed(1) ?? "0.0"}</span>
            <span className="text-xs text-gray-400">({product.reviewCount ?? product.reviews?.length ?? 0})</span>
          </div>
          ...
          <div className="flex items-center justify-between gap-2">
            <div>
              <span className="text-base font-700 text-gray-900">
                {formatPrice(product.price, settings.activeCurrency)}
              </span>
              {product.originalPrice && product.originalPrice > product.price && (
                <span className="text-xs text-gray-400 line-through ml-1.5">
                  {formatPrice(product.originalPrice, settings.activeCurrency)}
                </span>
              )}
            </div>
