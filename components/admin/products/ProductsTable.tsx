...
const mockProducts: AdminProduct[] = [
  { id: "1", name: "Organic Bananas", slug: "organic-bananas", sku: "FRU-001", category: "Fresh Fruits", price: 1.99, salePrice: 1.49, stock: 145, status: "ACTIVE", isFeatured: true, image: "https://images.unsplash.com/photo-1571771894821-ce9b6c11b08e?w=80&q=70", createdAt: "2026-01-15" },
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link
          ...
          className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shadow-lg shadow-emerald-500/20"
        >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-slate-800 border border-slate-700 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500"
          />
        </div>
          ...
          {["ALL", "ACTIVE", "DRAFT", "INACTIVE"].map(s => (
            <button
              ...
              className={`text-xs px-3 py-2 rounded-lg font-medium transition-colors ${
                statusFilter === s ? "bg-emerald-600 text-white" : "text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700"
              }`}
            >
              {s}
            </button>
      ...
      {/* Bulk actions */}
      {selectedIds.size > 0 && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 flex items-center gap-4">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {product.isFeatured && <span className="text-[10px] text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded">Featured</span>}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="relative flex items-center gap-1">
                      ...
                      <Link href={`/product/${product.slug}`} target="_blank"
                        className="w-7 h-7 rounded-lg bg-slate-800 hover:bg-emerald-500/20 hover:text-emerald-400 flex items-center justify-center text-slate-400 transition-colors">
