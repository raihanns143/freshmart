...
import { deleteProduct, updateDisplayOrder } from "@/lib/actions/admin-products";
...
export function ProductsManager({
  ...
  async function handleDisplayOrderChange(id: string, value: number) {
    startTransition(async () => {
      const result = await updateDisplayOrder(id, value);
      if (result.success) {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 flex-1 w-full">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-9 pr-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
              />
            </div>
            <select
              name="category"
              defaultValue={category ?? ""}
              className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50"
            >
            ...
            <button type="submit" className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-medium hidden md:block">
              Filter
            </button>
          </form>
        </div>
        ...
        {selected.size > 0 && (
          <div className="px-4 py-3 bg-blue-500/10 border-b border-blue-500/20 flex items-center justify-between">
            <span className="text-sm font-medium text-blue-400">{selected.size} products selected</span>
            <div className="flex gap-2">
              ...
              <tr className="border-b border-slate-800">
                <th className="px-4 py-3 text-left w-12">
                  <button onClick={toggleAll} className="text-slate-500 hover:text-white">
                    {selected.size === products.length && products.length > 0 ? <CheckSquare className="w-5 h-5 text-blue-500" /> : <Square className="w-5 h-5" />}
                  </button>
                </th>
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider">Home Order</th>
                    ...
                    <tr key={product.id} className="hover:bg-slate-800/50 transition-colors">
                      <td className="px-4 py-3">
                        <button onClick={() => toggle(product.id)} className="text-slate-500 hover:text-white">
                          {selected.has(product.id) ? <CheckSquare className="w-5 h-5 text-blue-500" /> : <Square className="w-5 h-5" />}
                        </button>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-slate-800 relative overflow-hidden flex-shrink-0 border border-slate-700">
                            {product.images[0] ? (
                              <Image src={product.images[0].url} alt={product.images[0].alt || ""} fill className="object-cover" unoptimized />
                            ) : (
                              <Package className="w-5 h-5 absolute inset-0 m-auto text-slate-500" />
                            )}
                          </div>
                          <div>
                            <Link href={`/admin/products/${product.id}/edit`} className="text-sm font-medium text-white hover:text-blue-400 transition-colors">
                              {product.name}
                            </Link>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${product.isActive ? "bg-blue-500/10 text-blue-400" : "bg-slate-800 text-slate-400"}`}>
                        ...
                        <input
                          ...
                          onBlur={(e) => {
                            ...
                            if (!isNaN(val) && val !== product.displayOrder) {
                              handleDisplayOrderChange(product.id, val);
                            }
                          }}
                          className="w-16 bg-slate-800 border border-slate-700 rounded text-white text-sm px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                      </td>
                      <td className="px-4 py-3 text-sm text-white">
                        {product.variants.length === 0 ? "—" : minPrice === maxPrice ? formatPrice(minPrice, settings.activeCurrency) : `${formatPrice(minPrice, settings.activeCurrency)} - ${formatPrice(maxPrice, settings.activeCurrency)}`}
                      </td>
