...
import {
  ...
  Clock, AlertTriangle, RefreshCw, ArrowUpRight, ArrowDownRight, Globe
} from "lucide-react";
import {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Revenue" value={formatPrice(stats.revenue, settings.activeCurrency)} change={12.5} icon={<DollarSign className="w-5 h-5 text-emerald-400" />} color="bg-emerald-500/10" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          ...
          <p className="text-3xl font-bold text-white">{formatPrice(stats.thisMonthRevenue, settings.activeCurrency)}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-white mb-4">Revenue & Orders – Last 7 Months (Mocked)</h3>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={salesData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
              <XAxis dataKey="month" tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ backgroundColor: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, fontSize: 12 }} labelStyle={{ color: "#94a3b8" }} />
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-white mb-4">Sales by Category (Mocked)</h3>
