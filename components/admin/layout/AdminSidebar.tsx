...
import {
  ...
  Activity, Database, LogOut, ChevronLeft, ShieldCheck, ChevronRight,
  Boxes
} from "lucide-react";
        ...
        <nav className="flex-1 overflow-y-auto py-4 space-y-0.5 px-2">
          {navItems.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon className={cn("w-4 h-4 flex-shrink-0", active ? "text-emerald-400" : "group-hover:text-white")} />
                {!collapsed && <span>{label}</span>}
                {!collapsed && active && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-emerald-400" />}
              </Link>
            );
          })}
        </nav>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 bg-slate-900 border-r border-slate-800 flex flex-col overflow-y-auto">
            <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-800">
              ...
              <div>
                <p className="font-bold text-white text-sm">FreshMart</p>
                <p className="text-emerald-400 text-xs">Admin Panel</p>
              </div>
            </div>
                ...
                return (
                  <Link key={href} href={href} onClick={() => setOpen(false)}
                    className={cn(
                      ...
                      active ? "bg-emerald-600/20 text-emerald-400 font-medium" : "text-slate-400 hover:text-white hover:bg-slate-800"
                    )}
                  >
