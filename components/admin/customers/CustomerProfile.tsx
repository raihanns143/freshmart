...
import { updateUserRole, deleteUser, blockUser, unblockUser } from "@/lib/actions/admin-customers";
...
export function CustomerProfile({ customer }: { customer: any }) {
  ...
  async function handleToggleBlock() {
    ...
    startTransition(async () => {
      if (isBlocked) {
        const result = await unblockUser(customer.id);
        if (result?.success) { setRole("USER"); toast.success("Customer unblocked"); }
        else toast.error(result?.error ?? "Failed");
      } else {
        const result = await blockUser(customer.id);
        if (result?.success) { setRole("BLOCKED"); toast.success("Customer blocked"); }
        else toast.error(result?.error ?? "Failed");
      }
    });
  }
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: "Total Spent", value: formatPrice(totalSpent, settings.activeCurrency), icon: ShoppingBag },
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {customer.orders.map((order: any) => (
                      <tr key={order.id} className="hover:bg-slate-800/50">
                        ...
                        <td className="px-4 py-3 text-sm font-semibold text-white">{formatPrice(order.total, settings.activeCurrency)}</td>
